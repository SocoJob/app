...
class JobController extends AppBaseController
{
    ...
    public function jobDetails($uniqueJobId)
    {
        ...
        $relatedJobs = Job::with('jobCategory', 'jobShift',
            'company')->whereJobCategoryId($job->job_category_id)->whereDate('job_expiry_date', '>=',
    ...
    public function saveFavouriteJob(Request $request)
    {
        ...
        if ($favouriteJob) {
            return $this->sendResponse($favouriteJob, 'Trabajo favorito fue agregado con éxito.');
        }

        return $this->sendResponse($favouriteJob, 'Trabajo favorito fue removido con éxito.');
    }
