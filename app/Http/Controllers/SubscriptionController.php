...
class SubscriptionController extends AppBaseController
{
    ...
    public function purchaseSubscription(Request $request)
    {
        ...
        setStripeApiKey();
        $session = Session::create($arraySession);
        $result = [
    ...
    public function paymentSuccess(Request $request)
    {
        ...
        Flash::success('La suscripción se ha realizado correctamente.');
    ...
    public function changeTransactionStatus(Request $request)
    {
        ...
        //Modificado por Rpodrigo para plan anual
    ...
    public function paymentCandidateStripe(Request $request){
        $idCliente = $request->input('candidate');
        // $key = 'sk_test_51LkxsVA1sFneOYD6jey9VB6xgMzhzHH6lsR7AcCCUiCO3NJXeOb5eAouoH4Zg82DzUFxmDYKuyJmjgccrfBgBUsQ00aukdEanj';
        $key = env('STRIPE_SECRET');
        $price_id_candidate = env('PRICEID_CANDIDATE');
        $iva_id = env('IVAID');
        // Crear un pago en Stripe con el ID del cliente como metadato
        ...
        $session = Session::create([
            ...
            'line_items' => [
              [
                'price' => $price_id_candidate, // ID del precio en Stripe
                'quantity' => 1,
                'tax_rates' => [$iva_id]
              ],
            ],
            'mode' => 'payment',
            'success_url' => route('payment.candidate.success', [ 'id' => $idCliente] ),
            'cancel_url' =>  url('employer/failed-payment?error=payment_cancelled'),
            'metadata' => [
              'id_cliente' => $idCliente, // Agregar el ID del cliente como metadata
            ],
          ]);
        // Redirigir a la página de confirmación de pago de Stripe
        //   dd($session->url);
        return response()->json(['url' => $session->url]);
    }
    public function paymentCandidateSuccess($id)
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $stringGenerado .= $caracteres[$indice];
        }
        $candidate->token = $stringGenerado;
        $candidate->save();
        return redirect()->route('front.candidate.details',[$candidate->unique_id,$stringGenerado]);
