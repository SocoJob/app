    ...
    public function searchCandidates()
    {
        ...
        $query->when($this->searchByCandidate != '' && $this->searchBy == 'byName', function (Builder $q) {
            $q->whereHas('user', function (Builder $query) {
                ...
                $query->orWhere('last_name', 'like', '%'.strtolower($this->searchByCandidate).'%');
                $query->orWhereHas('country', function (Builder $q) {
                    $q->whereRaw('lower(name) like ?', ['%'.strtolower($this->searchByCandidate).'%']);
                });
...
//        });
        $query->when($this->location != '', function (Builder $q) {
            $q->whereHas('user', function (Builder $query) {
                $query->whereHas('candidateSkill',function(Builder $query2){
                    $query2->whereRaw('lower(name) like ?', ['%'.strtolower($this->location).'%']);;
                });
            });
        });
