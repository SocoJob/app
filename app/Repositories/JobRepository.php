...
class JobRepository extends BaseRepository
{
    ...
    public function prepareJobData()
    {
        ...
        $data['genders'] = JobType::toBase()->pluck('name','id');
        $data['careerLevels'] = JobCategory::toBase()->pluck('name', 'id');//CareerLevel::toBase()->pluck('level_name', 'id');
        $data['functionalAreas'] = JobShift::toBase()->pluck('shift', 'id');  
    ...
    public function store($input)
    {
        try {
            ...
            $input['job_expiry_date'] = $nueva_fecha;
            /** @var Job $job */
        ...
        if (! $favouriteJob) {
            ...
            NotificationSetting::where('key','FOLLOW_JOB')->first()->value == 1 ?
                addNotification([
                    ...
                    $loggedInUser->first_name.' '.$loggedInUser->last_name.' empezó a seguir '.$job->job_title.'.',
                ]) : false;
