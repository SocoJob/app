...
function loadAppliedJobsData() {
    ...
    document.addEventListener('notDeleted', function () {
        swal({
            icon: 'error',
            type: 'error',
            title: Lang.get('messages.flash.not_deleted'),
            text: Lang.get('messages.flash.job_cant_delete'),
            buttons: {
                confirm:Lang.get('messages.common.ok'),
            },
            reverseButtons: true,
                    ...
                    if (result.data.rejectedSlot) {
                        if (!isEmpty(result.data.employer_cancel_note)) {
                            $('#scheduleSlotBookValidationErrorsBox').removeClass('d-none')
                                .html(result.data.company_fullName + Lang.get('messages.job_stage.cancel_your_selected_slot') + '<br>' + '<b>Reason</b>:- ' + result.data.employer_cancel_note);
